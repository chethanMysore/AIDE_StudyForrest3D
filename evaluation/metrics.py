...
class FocalTverskyLoss(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        y_true_pos = torch.flatten(y_true,1)
        y_pred_pos = torch.flatten(y_pred,1)
        true_pos = torch.sum(y_true_pos * y_pred_pos,1)
        false_neg = torch.sum(y_true_pos * (1 - y_pred_pos),1)
        false_pos = torch.sum((1 - y_true_pos) * y_pred_pos,1)
