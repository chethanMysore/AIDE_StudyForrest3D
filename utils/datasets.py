...
class prostate_seg(Dataset):
    ...
    def __init__(self, root, csv_file, tempmaskfolder, train=True, transform=None, imgs=[], masks=[]):
        self.root = root
        ...
        if len(self.imgs) == 0 or len(self.masks) == 0:
            print("imgaes or masks are unavailable in the csv file!!")
            sys.exit()
    ...
    def __getitem__(self, idx):
        ...
        imgarr = sitk.GetArrayFromImage(sitk.ReadImage(os.path.join(self.root, str(self.imgs[idx]))))
